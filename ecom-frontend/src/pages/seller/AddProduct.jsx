...
export default function AddProduct({ onClose, isEditMode = false, product = null }) {
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-sm border">
            ...
            {/* Specifications Section — Dynamic based on selected micro category */}
            {(microCategory || (isEditMode && product?.categoryId)) && (
                <section className="mb-10">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Fill in the specification fields specific to this product category. Fields marked with <span className="text-red-500">*</span> are required.</p>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                        <DynamicSpecForm
                            categoryId={microCategory || product?.categoryId}
                            onSpecsChange={setSpecifications}
                            initialSpecs={isEditMode ? (product?.specifications || {}) : {}}
                        />
                    </div>
                </section>
            )}
